...
```{r,echo=FALSE}
# Gemeinsame Funktionen für alle Diagramme
add_labels <- function(plot, m_gg, p_gg, p_prohibitiv = NULL, m_Sattigung = NULL) {
  plot <- plot +
    geom_point(aes(x = m_gg, y = p_gg), color = "red", size = 3) +
    annotate("text", x = m_gg, y = p_gg, label = paste0("(", round(m_gg, 2), ", ", round(p_gg, 2), ")"), hjust = -0.1, vjust = -0.5)
  
  if (!is.null(p_prohibitiv)) {
    plot <- plot +
      geom_hline(yintercept = p_prohibitiv, linetype = "dashed", color = "gray") +
      annotate("text", x = 0, y = p_prohibitiv, label = "p_prohibitiv", hjust = -0.1, vjust = -0.5)
  }
  
  if (!is.null(m_Sattigung)) {
    plot <- plot +
      geom_vline(xintercept = m_Sattigung, linetype = "dashed", color = "gray") +
      annotate("text", x = m_Sattigung, y = 0, label = "m_Sättigung", hjust = -0.1, vjust = -0.5)
  }
...
```{r}
# (a)
m <- seq(0, 700, length.out = 100)
p_N <- -2 * m + 700
p_A <- 0.5 * m + 125

df <- data.frame(m = m, p_N = p_N, p_A = p_A)

plot_a <- ggplot(df, aes(x = m)) +
  geom_line(aes(y = p_N, color = "Nachfrage")) +
  geom_line(aes(y = p_A, color = "Angebot")) +
  scale_color_manual(values = c("Nachfrage" = "blue", "Angebot" = "red")) +
  labs(x = "Menge", y = "Preis", title = "Angebots- und Nachfragekurven (a)") +
  theme_minimal()

plot_a <- add_labels(plot_a, 230, 240, 350, 700)
print(plot_a)
```
...
```{r}
p <- seq(0, 400, length.out = 100)
m_N <- -2 * p + 800
m_A <- p - 20

df <- data.frame(p = p, m_N = m_N, m_A = m_A)

plot_b <- ggplot(df, aes(y = p)) +
  geom_line(aes(x = m_N, color = "Nachfrage")) +
  geom_line(aes(x = m_A, color = "Angebot")) +
  scale_color_manual(values = c("Nachfrage" = "blue", "Angebot" = "red")) +
  labs(y = "Preis", x = "Menge", title = "Angebots- und Nachfragekurven (b)") +
  theme_minimal()

plot_b <- add_labels(plot_b, 253.33, 273.33, 400, 800)
print(plot_b)
```
...
```{r}
# (c)
m <- seq(0, 30, length.out = 100)
p_N <- 30 - m
p_A <- 10 * m

df <- data.frame(m = m, p_N = p_N, p_A = p_A)

plot_c <- ggplot(df, aes(x = m)) +
  geom_line(aes(y = p_N, color = "Nachfrage")) +
  geom_line(aes(y = p_A, color = "Angebot")) +
  scale_color_manual(values = c("Nachfrage" = "blue", "Angebot" = "red")) +
  labs(x = "Menge", y = "Preis", title = "Angebots- und Nachfragekurven (c)") +
  theme_minimal()

plot_c <- add_labels(plot_c, 2.73, 27.27, 30, 30)
print(plot_c)
```
...
plot_d <- ggplot(df, aes(x = m)) +
  geom_line(aes(y = p_N, color = "Nachfrage")) +
  geom_line(aes(y = p_A, color = "Angebot")) +
  scale_color_manual(values = c("Nachfrage" = "blue", "Angebot" = "red")) +
  labs(x = "Menge", y = "Preis", title = "Angebots- und Nachfragekurven (d)") +
  theme_minimal()

plot_d <- add_labels(plot_d, 5, 780, 1080, sqrt(1080/12))
print(plot_d)
```
...
```{r}
m <- seq(0, 80, length.out = 100)
p_N <- (80 - m) / 4
p_A <- rep(17, 100)

df <- data.frame(m = m, p_N = p_N, p_A = p_A)

plot_e <- ggplot(df, aes(x = m)) +
  geom_line(aes(y = p_N, color = "Nachfrage")) +
  geom_line(aes(y = p_A, color = "Angebot")) +
  scale_color_manual(values = c("Nachfrage" = "blue", "Angebot" = "red")) +
  labs(x = "Menge", y = "Preis", title = "Angebots- und Nachfragekurven (e)") +
  theme_minimal()

plot_e <- add_labels(plot_e, 12, 17, 20, 80)
print(plot_e)
```
...
plot_f <- ggplot(df, aes(x = m)) +
  geom_line(aes(y = p_N, color = "Nachfrage")) +
  geom_line(aes(y = p_A, color = "Angebot")) +
  scale_color_manual(values = c("Nachfrage" = "blue", "Angebot" = "red")) +
  labs(x = "Menge", y = "Preis", title = "Angebots- und Nachfragekurven (f)") +
  theme_minimal()

plot_f <- add_labels(plot_f, 16, 7, 15, 30)
print(plot_f)
```
...
```{r}
# (g)
m <- seq(0, 100, length.out = 100)
p_N <- ((100 - m) / 5)^2
p_A <- 4 * m + 80

df <- data.frame(m = m, p_N = p_N, p_A = p_A)

plot_g <- ggplot(df, aes(x = m)) +
  geom_line(aes(y = p_N, color = "Nachfrage")) +
  geom_line(aes(y = p_A, color = "Angebot")) +
  scale_color_manual(values = c("Nachfrage" = "blue", "Angebot" = "red")) +
  labs(x = "Menge", y = "Preis", title = "Angebots- und Nachfragekurven (g)") +
  theme_minimal()

plot_g <- add_labels(plot_g, 128.28, 593.14, 400, 100)
print(plot_g)
```
